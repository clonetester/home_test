...
// Initialize Firebase
  ...
  database.ref('/data').once('value').then(function(snapshot) {
    ...
    var url = 'https://docs.google.com/spreadsheets/d/'+docsId+'/gviz/tq?tqx=out:json&tq&gid='+pageDocs;
    fetch(url)
...
document.getElementById('submitTeleHeader').addEventListener('click', () => {
    ...
    if(data.room=='Loại phòng' || data.dateCheckIn == '' || data.dateCheckOut == '' || data.name == ''|| data.phone == ''){
    ...
    axios.post(`https://api.telegram.org/bot${telegramBotToken}/sendMessage`, {
      chat_id: chatId,
      text: 'Lựa chọn: '+ data.room + '\nNgày checkin: ' + data.dateCheckIn + '\nNgày checkout: '+ data.dateCheckOut + '\nTên khách hàng: '+ data.name + '\nSố điện thoại: '+ data.phone
    })
    .then((response) => {
      alert('Đặt chỗ thành công.');
    })
    .catch((error) => {
      alert('Đặt chỗ thất bại. Hãy liên hệ chúng tôi theo số điện thoại để tiếp tục.');
